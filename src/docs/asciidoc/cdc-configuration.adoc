...
The offset is stored using a key value of `reader.readerName` property, which enables multiple readers to use the same store.
...
| Identificates reader. Also used for the mysql binlog profile as key to store the current offset in the offset store.
| -
| -
...
Note that reader name is used for mysql binlog profile as key to store the current offset in the offset store.
