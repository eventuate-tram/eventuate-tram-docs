...
| eventuateio/eventuate-cdc-service:0.3.0.RELEASE
...
But it also supports Apache ActiveMQ, RabbitMQ, Redis.
...
| Number of partitions. Messages are split between partitions similar to kafka.
...
The `EVENTUATELOCAL_CDC_READ_OLD_DEBEZIUM_DB_OFFSET_STORAGE_TOPIC` property configures the UCDC to Debezium-maintained offset from the old offset store Apache Kafka topic.
...
==== Upgrading to separate cdc

To upgrade eventuate local or eventuate tram to eventuate cdc service
replace image to eventuateio/eventuate-cdc-service:0.3.0.RELEASE.

To use eventuate cdc as eventuate local cdc add variable EVENTUATE_CDC_TYPE: EventuateLocal.
By default it uses tram cdc.
