...
* 64 bits - DB generated ID
* 16 bits - counter, used for internal needs, usually zero
* 48 bits - a unique Eventuate outbox id

Each `MESSAGE` and `EVENTS` table must have an Eventuate Outbox ID that is globally unique (within your application).
All producers that insert messages/events into one of those tables and the CDC reader that reads from the table must be configured with the same Eventuate outbox ID.
