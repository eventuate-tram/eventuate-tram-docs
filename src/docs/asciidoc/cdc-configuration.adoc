...
| Identificates reader. Used for the mysql binlog profile as key to store the current offset in the offset store if eventuatelocal.cdc.offset.store.key is not specified.
| -
| -
