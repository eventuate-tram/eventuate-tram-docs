...
* Multi-reader/multi-pipeline support - rather than reading from either a single Eventuate Local `EVENTS` table or Eventuate Tram `MESSAGE` table, the new CDC have read from multiple `EVENTS` and/or `MESSAGE` tables.
