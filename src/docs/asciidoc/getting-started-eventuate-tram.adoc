...
eventuate.database.schema=eventuate
----
The `eventuate.database.schema` property is optional.
You can use it to define database schema where Eventuate tables are located
(Please read link:cdc-configuration.html[CDC Configuration document]).
By default `eventuate` schema is used. Specify `NONE` to use schema from jdbc connection.
